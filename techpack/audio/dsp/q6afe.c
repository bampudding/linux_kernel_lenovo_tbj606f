...
#define AFE_PORT_ID_AWDSP_RX			(AFE_PORT_ID_PRIMARY_TDM_RX)
#define AFE_PORT_ID_AWDSP_TX			(AFE_PORT_ID_PRIMARY_TDM_TX)
...
int afe_get_topology(int port_id)
{
	...
	pr_err("%s: [Awinic] port_id [0x%x], index [0x%x]", __func__, port_id, port_index);
...
static int afe_send_port_topology_id(u16 port_id)
{
	...
	pr_info("%s: Awinic AFE set topology id 0x%x  enable for port 0x%x ret %d ,index [0x%x]\n",
			__func__, topology_id, port_id, ret, index);
