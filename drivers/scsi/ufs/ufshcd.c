...
#define UFSHCD_CLK_GATING_DELAY_MS_PWR_SAVE	80
#define UFSHCD_CLK_GATING_DELAY_MS_PERF		80
					...
					unsigned long completed_reqs)
{
	...
	for_each_set_bit(index, &completed_reqs, hba->nutrs) {
		...
		if (cmd) {
			...
			/*
			 * Release clock/hibern8 holds only after lrb_in_use is
			 * cleared so the final request can schedule gating while
			 * keeping the LRB owned until all completion bookkeeping
			 * and vendor request cleanup has finished.
			 */
...
static int ufshcd_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (!ufshcd_is_ufs_dev_active(hba)) {
		...
		if (ret) {
			/*
			 * Error recovery may have restored the device and link while
			 * START STOP UNIT was failing in this resume path.
			 */
			if (ufshcd_is_ufs_dev_active(hba) &&
			    ufshcd_is_link_active(hba))
				ret = 0;
				...
				ret = 0;
				dev_info(hba->dev,
					 "dev pwr mode=%d, UIC link state=%d\n",
