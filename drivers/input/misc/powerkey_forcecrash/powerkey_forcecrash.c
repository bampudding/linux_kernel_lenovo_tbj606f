...
{
    pr_info("Received event: type=%u, code=%u, value=%d\n", type, code, value);
    pr_info("Looking for value: %d ...", KEY_POWER);
	switch (code) {
	case KEY_POWER:
...
MODULE_DESCRIPTION("Force crash on power key long press of 3 secs");
