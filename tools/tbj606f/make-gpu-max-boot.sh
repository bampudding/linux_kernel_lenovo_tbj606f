...
# Known-good source boot, kernel shared between control/busy600/max950 images;
# private temporary fastboot boot experiment only. No partition writes.
set -euo pipefail
if [[ $# != 4 || ( ${4:-} != stock && ${4:-} != 600 && ${4:-} != 950 ) ]];then
  echo 'Usage: make-gpu-max-boot.sh VERIFIED_STABLE_BOOT.img Image OUTPUT_BOOT.img stock|600|950' >&2
  exit 2
fi
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/p11-gpubusy.XXXXXXXX")
partial="$out.partial.$$"
...
for ((i=0; i<${#options[@]};i++));do
  if [[ ${options[i]} == --cmdline ]];then
    [[ ${options[i+1]} != *p11.f=* && ${options[i+1]} != *p11.gw=* && ${options[i+1]} != *p11.gb=* && ${options[i+1]} != *p11tune.gpu_top_freq=* ]] || { echo 'unsafe experiment already in source boot cmdline' >&2;exit 1; }
    if [[ $mode == 600 || $mode == 950 ]];then options[i+1]="${options[i+1]} p11.gb=$mode";fi
    found=1
  fi
done
...
if [[ $mode == 600 || $mode == 950 ]];then
 [[ $verified == *p11.gb=$mode* ]] || { echo 'busy boost option absent' >&2;exit 1; }
else
 [[ $verified != *p11.gb=* ]] || { echo 'unexpected busy boost option' >&2;exit 1; }
fi
mv "$partial" "$out"
sha256sum "$out"
