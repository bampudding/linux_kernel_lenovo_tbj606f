...
# Known-good source boot, kernel shared between control/busy600 images;
# private temporary fastboot boot experiment only. No partition writes.
set -euo pipefail
if [[ $# != 4 || ( ${4:-} != stock && ${4:-} != 600 ) ]];then
  echo 'Usage: make-gpu-busy-boot.sh VERIFIED_STABLE_BOOT.img Image OUTPUT_BOOT.img stock|600' >&2
  exit 2
fi
...
for ((i=0; i<${#options[@]};i++));do
  if [[ ${options[i]} == --cmdline ]];then
    ...
    if [[ $mode == 600 ]];then options[i+1]="${options[i+1]} p11.gb=600";fi
    found=1
  fi
done
...
if [[ $mode == 600 ]];then
 [[ $verified == *p11.gb=600* ]] || { echo 'busy boost option absent' >&2;exit 1; }
else
