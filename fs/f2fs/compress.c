...
static int zstd_compress_pages(struct compress_ctx *cc)
{
	...
	outbuf.size = cc->rlen - PAGE_SIZE - COMPRESS_HEADER_SIZE;

	ret = ZSTD_compressStream(stream, &outbuf, &inbuf);
	if (ZSTD_isError(ret)) {
		printk_ratelimited("%sF2FS-fs (%s): %s ZSTD_compressStream failed, ret: %d\n",
				KERN_ERR, F2FS_I_SB(cc->inode)->sb->s_id,
				__func__, (int)ZSTD_getErrorCode(ret));
		return -EIO;
	}
	...
	/* Output buffer was insufficient; fall back to raw cluster write. */
	if (ret)
		return -EAGAIN;
...
static int zstd_decompress_pages(struct decompress_io_ctx *dic)
{
	...
	if (ZSTD_isError(ret)) {
		printk_ratelimited("%sF2FS-fs (%s): %s ZSTD_decompressStream failed, ret: %d\n",
				KERN_ERR, F2FS_I_SB(dic->inode)->sb->s_id,
				__func__, (int)ZSTD_getErrorCode(ret));
		return -EIO;
	}
	...
	if (dic->rlen != outbuf.pos) {
		printk_ratelimited("%sF2FS-fs (%s): %s ZSTD invalid rlen:%zu, expected:%lu\n",
				KERN_ERR, F2FS_I_SB(dic->inode)->sb->s_id,
				__func__, outbuf.pos,
				PAGE_SIZE << dic->log_cluster_size);
		return -EIO;
...
out_free_dic:
	f2fs_decompress_end_io(dic->rpages, dic->cluster_size, ret, false);
