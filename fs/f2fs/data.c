...
{
	if (noio) {
		/* No failure on bio allocation: bioset reserve backs this path. */
...
static void __read_end_io(struct bio *bio, bool compr)
{
	...
	bio_for_each_segment_all(bv, bio, i) {
		...
		if (compr && f2fs_is_compressed_page(page)) {
			f2fs_decompress_pages(bio, page, false);
			continue;
		}
#endif
...
#endif
		if (!target->mapping) {
			target = fscrypt_control_page(target);
			if (IS_ERR(target))
				continue;
		}
...
static struct bio *f2fs_grab_read_bio(struct inode *inode, block_t blkaddr,
				unsigned nr_pages, unsigned op_flag, bool for_write)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	struct bio *bio;
	struct bio_post_read_ctx *ctx;
